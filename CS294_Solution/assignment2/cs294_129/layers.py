...
def batchnorm_backward_alt(dout, cache):
  ...
  dx = gamma * (N * dout / denom - np.sum(dout, 0)/denom
                        - xhat * np.sum(xhat*dout, 0) / denom)/ N
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
...
def max_pool_forward_naive(x, pool_param):
  ...
  for n in range(N):
      ...
      for c in range(C):
          ...
          for i in range(H1):
              for j in range(W1):
                  out[n, c, i, j] = np.max(x[n, c, i*s:i*s + p_h, j*s:j*s + p_w])
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  cache = (x, pool_param)
...
def max_pool_backward_naive(dout, cache):
  ...
  for n in range(N):
      for c in range(C):
          for i in range(H1):
              for j in range(W1):
                  x_mp = x[n, c, i*s:i*s+ p_h, j*s:j*s + p_w]
                  max_tmp = np.max(x_mp)
                  x_mask = x_mp == max_tmp
                  dx[n, c, i*s:i*s+ p_h, j*s:j*s + p_w] += dout[n, c, i, j] * x_mask
  #############################################################################
  #                             END OF YOUR CODE                              #
  #############################################################################
  return dx
